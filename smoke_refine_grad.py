"""smoke：GeoEnergy ＋ 主人的更新式。⭐ 每格都寫明【為什麼期望是這個答案】。"""
import sys
sys.path.insert(0, "/home/cymaxwelllee/Projects/lacot")
import numpy as np, torch
from lacot.refine_grad import GeoEnergy, grad_refine, _clip
...
e1 = geo(dec(u_fix), *sg)
print(f"case4  修正前 E {e0.mean():.3f} → 修正後 {e1.mean():.3f}   ({e1.mean()-e0.mean():+.3f})")
print(f"       穿牆 {hist[0]['wall']:.3f}→{hist[-1]['wall']:.3f}"
      f"   離終點 {hist[0]['goal']:.3f}→{hist[-1]['goal']:.3f}"
      f"   log p {hist[0]['logp']:.1f}→{hist[-1]['logp']:.1f}")
# ⭐ 2026-08-29 energy 語義（主人裁）：E 越小越好 ⇒ 修正要讓 E【下降】
if e1.mean() >= e0.mean():
    fails.append(f"case4 修正沒有讓 E 下降（{e0.mean():.3f}→{e1.mean():.3f}）⇒ 更新式方向錯了")
