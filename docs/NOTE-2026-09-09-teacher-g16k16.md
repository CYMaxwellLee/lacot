...
12. **「同量級」的數字門檻（0.001 < mse3 < 0.1）是本檔自訂，不是工單指定**
    ——工單§驗收①只寫「同量級即可」，沒給精確數字範圍，這個寬鬆區間是我
    自己拍的（拍的原因：val MSE=.0153，上下一個數量級大致落在
    .0015~.153，取了更寬鬆的 .001~.1 當粗篩），不是量出來的、如實標註。
    ⚠️ 但這格結論不是只靠這個自訂門檻撐著——同一步驟另外做的「加驗」
    （整個 val 切分精確比對 JSON 記錄值，差=0.00e+00）是量出來的精確
    重現，比這個自訂門檻強得多，載入正確性的結論主要靠加驗那格，這個
    寬鬆門檻只是工單要求的最低限度動作。
13. 沒做到的「查不到／跑不出」項目：無。ckpt／obs 統計量／ruler_pack／
