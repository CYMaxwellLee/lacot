...
- ✅ 8/30 前哨已做（`LACOT_DEC_ANCHOR` eval-time 平移錨定、零重訓）：
  1. 抓到 conf2 的 tau 自校準前提被錨定破壞（頭端分散人工歸零 ⇒ 門檻無限嚴 ⇒ 挑點塌腳邊
     0.37）— 修法＝calib 窗挪中段（subgoal.py 已修、預設行為不變）。
  2. 🚨 更深的發現：修完校準後挑點仍兩極化（中位 0.67、17% 飛 22）⇒ 錨定把「位置錯」
     修掉後露出第二層病：**形狀塌縮**（殘骸 M 份路總弧長太短、贴地繞）。
  ⇒ 93% 病正式分解成【位置錯（平移可修）＋形狀縮（只有資料引擎能修）】兩層 —
  eval-time 錨定單獨不足以當 P1a 正式方案（⛔ 別拿它跑 n=100 正式格），
  P1a 的訓練版結構錨定照舊跟 P1b 合併重訓；病理學那節多了一個乾淨的分解敘事。
