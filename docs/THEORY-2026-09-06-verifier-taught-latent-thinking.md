...
1. **劑量四點平坦**：除 $p{=}0$ 外，$p\in\{.05,.1,.3\}$ 的效用增益全在噪音級
   ...
   ⭐ **〔9/6 開獎 — 證偽，且比「平坦」更糟〕**：8000 步家族（ep200 錨、新分母 .137）四點
   $\mathrm{Int}(.05/.1/.3)=\mathbf{-.26/-.35/-.05}$ — **全在預測區間外、全部反號**
   （機制＝殘廢效應：zero 全低於無 intent 基線）⇒ 平衡版點估**證偽**。⛔ 不是「平坦被證實」
   ——「全平」與「全負」是兩件事【F⑤@0906；登錄 postA1 **E6.1**】。
2. **劑量比值封頂**：$\mathrm{Int}(.1)/\mathrm{Int}(.3)\le3$、$\mathrm{Int}(.05)/\mathrm{Int}(.3)\le6$，
   ...
   ⭐ **〔9/6 開獎 — 定義域失效，⛔ 不是被推翻〕**：四點全負 ⇒ 比值成負比負、封頂命題
   **無可判內容**；一行證（代數）仍為真，失效的是「達平衡且 $\mathrm{Int}>0$」這個讀法前提
   ⇒ 正確處置＝**標定義域**【F⑤@0906；postA1 **E6.2**】。
3. **劑量形狀三性質＋單點分版**：$\mathrm{Int}(p)$ 單調降、凸（斜率遞增）兩版共有；
   ...
   ⚠️〔9/6 深審 S5〕行內的區間值 $[0.2,0.5]$ **依賴 $\kappa T$ 重校準**（transient 帶由已被
   訓練曲線否定的校準推出）⇒ **判讀先過夜審 §5 的重校準關**，⛔ 別直接讀區間值
   【REVIEW-0905 §5；postA1〔追加勘誤 **E4**〕】。
   ⭐ **〔9/6 開獎 — 判別器判了，transient 版勝出；但不是被這一點判的〕**：性質 1 單調
   **倒**（$-.26/-.35/-.05$ 非單調 ⇒ §4.4 自訂逃生條款「非單調＝兩版全倒」觸發）；性質 2
   凸性不等式數值上成立（$1.8\ge-1.5$）但在非單調＋全負下**無判讀力**；性質 3 的
   $\mathrm{Int}(.05)=-.26$ **字面**落「平衡版活」格 — ⛔ **不可如此讀**（三格是為
   $\mathrm{Int}>0$ 寫的、$-.26$ 在樹的定義域外，且該帶已由 E4 標疑）。**實際判它的是 $T$ 軸**：
   同 $p{=}.3$ 練到 11429 步，zero 超額 $-.007\to\mathbf{+.086}$、批級 R0 $+.105$（~1.5σ），
   step-matched `[單顆 s40]` Int(idpxm)=1.10／Int(idp01L)=0.88 ⇒ **transient／慢動力學版勝出**
   【F①⑤⑧@0906；postA1 **E6.3–E6.4**】。⛔ 勝出的是**讀法**、不是 §4.3 那組 $\kappa T$ 數字
   （E4 標疑不撤）。
...
6. **rung 0 headroom 判準（雙錶版）**：stitch 題集上 pass@G − pass@1 gap 與 reward headroom
   至少一個顯著 $>0$ ⇒ GRPO 有物可放大〔**單向訊號**〕。⛔〔9/6 深審 M1〕**反向不成立** —
   gap$=0$ **不是**「幫浦無輸入」的定理（dense reward 下群內 reward 仍可有變異 ⇒
   $\hat A\ne0$）；零輸入的**定理級**判準只有一把：**退化群比例 $=1$**（群內 reward 全同
   ⇒ $\hat A\equiv0$、L4(ii) 字面義）。⇒ 降級整臂要**兩錶都讀**，⛔ 別按單錶砍臂
   （門檻【猜測】）【設計卡 §3.4、§4 rung 0；帳本 §4 表 BoN／GRPO 行、§5】。
7. **WS（warm-start）雙判**：黏住 ⇔ 塌回（判退火藥是否單獨可行、Conj 3.2）【內化 §3.1；
   F⑭】；判讀尺＝slaved 域斜率比 $\mu_-=2.759\pm20\%$ 帶、⛔ 不用 2.3 定值
   〔⑮ 收緊①、toy 精度驗訖〕【F⑮】。
   ⚠️ **〔9/6 開獎 — 此尺已斷〕**：④ 實測兩臂 divlog 斜率比 **0.990**（idp0.1
   $-2.112\times10^{-4}$/step vs idp0.3 $-2.133\times10^{-4}$/step）**落在 $[2.21,3.31]$ 帶外**，
   且預釘樹兩岔（「斜率 0」「比值對上」）**皆未中** — 落第三態：衰減在、$p$ 不驅動
   （散度衰減是 $p$ 無關的 generic 過程）。⛔ **別拿它判 WS 療程八顆**；**療程判讀改用
   配對差（on−zero）與 zero 水位** — ⑥ 續訓格即此讀法（f27n@8000 起 $+p{=}.3$ 續 4000 步 ⇒
   zero .552／on .556、配對差 .004，`[單顆 s40]`；純續訓 zero 全崩＝③）
   【F④⑥@0906；postA1 **E6.5**】。⭐ 雙判的**黏住/塌回**本體不受影響 — 斷的只有那把尺。
...
9. **one-shot 連續自生 u 增益 ≈0**（可被吸收成 flow 的噪音維）⇒ rung 3 要嘛迭代、
   ...
   ⚠️〔9/6 深審 S3〕**限定詞**：「可吸收」是 **population／表達力**語意，⛔ **不排除優化紅利** —
   有限訓練下 $f_\theta$ 自帶預訓練結構，把 $\varepsilon$ 直接餵 flow 要它重學該變換，增益可
   經**優化難度差**為正（＝R2.5「計算捷徑」的形狀）。⇒ 「增益 $\approx0$」要另加隱含假設
   「**優化紅利歸零**」才成立；量到正增益時**先判它是表達力還是優化紅利**，⛔ 別直接讀成
   P3 證偽【分類 §3.2 C1 註、§5.2 P3 隱含假設欄】。
