...
**〔9/5 深夜・實證狀態插播〕**：A1 已被直接量測**證偽於本資料**（I(τ;a|s,g)≈2.5 bits、
FINDINGS ⑰）⇒ Prop 2.1／2.3 降為「A1 極限情形」的理論錨、非本資料的實況；鎖死（⑬ 實錘）
改讀為**動力學陷阱**（R2.4 賽跑＋丁 overshoot 為主敘事、與 η>0 相容）；藥方分工不變、
權重移向退火 p＋L_div。§3.3 的 C-i 在本資料已部分成立 — 多路線構造的角色轉為 C-iii。
