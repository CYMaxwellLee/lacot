...
| 11 | 中繼點選法：固定弧長 7.5 vs 信心選點 | 待實作 | 主人 8/29 提案。「信心」候選定義：(a) 同題多抽幾份 u、選多條計畫的【共識點】（跟 D4 多樣性量測共用機制、成本低）；(b) 候選短程 cond 的 flow 密度；(c) GeoEnergy 局部安全度。7.5 的現行理由＝訓練分布路長中位（短程坐在資料最肥處），是信心選點的固定近似 |
