...
# ⚠️ 重跑臂要是【真的重跑】：同一顆模型換一條 action-noise stream ⇒ 少數題會翻面。
#    ⛔ 不可以直接把受測臂本人放進去 —— 那是 case6 在抓的退化輸入。
#    ✅ 2026-08-30 主人裁定後 spec_pair 預設 = ("lacot","lacot_rerun")（受測對象要會抽樣）；
#       這裡的合成 rows 只是標籤，拿 bc 那份充當 lacot 臂的數值沒有語義問題。
lc_rr = rows([1] * 268 + [0, 0] + [1, 1] + [0] * 28)   # 4 題不一致、2 比 2
res3 = sanity_check({"random": rnd, "bc": bc, "lacot": bc, "lacot_rerun": lc_rr})
...
res4 = sanity_check({"random": rnd, "bc": weak, "lacot": weak, "lacot_rerun": weak_rr})
...
#    ✅ 2026-08-30 主線已換臂（lacot 重跑），但這個防呆分支要繼續會叫 ——
#       它抓的是「受測 arm 其實沒在抽樣」這一族的退化，不限 bc。
res6 = sanity_check({"random": rnd, "bc": bc, "lacot": bc, "lacot_rerun": bc})   # ⛔ 兩臂是同一份 rows
...
if not sanity_check({"random": rnd, "bc": bc, "lacot": bc,
                     "lacot_rerun": lc_rr})["gates"]["specificity"]:
