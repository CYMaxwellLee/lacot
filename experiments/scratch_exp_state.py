"""LaCoT on STATE input (master's call): e_target compresses the future POSITION trajectory,
encoder aligned to SOTA (MLP 512/512, GELU+LayerNorm, xavier -- like train_value_official).
Non-reconstruction objective = contrastive: (s,g) query <-> trajectory e_target key, InfoNCE.
Loads the state pointmaze npz directly (obs = 2D position); simple geometric hindsight goals.

Metrics: match-acc (>> chance = encodes) ; key-offdiag-sim (low = distinct = no collapse).
"""
import os, sys, time
import numpy as np
import torch
...
d = np.load(DATA)
OBS = np.asarray(d["observations"], np.float32)   # (N, 2)
...
print(f"state obs {OBS.shape}, {len(ends)} trajectories; pos range {OBS.min(0)}..{OBS.max(0)}", flush=True)

B, T_CAP, D, D_MODEL, K, GEOM_P = 64, 16, 128, 256, 64, 0.02
...
enc = sota_mlp(2, 512, 512, 2).to(device)
pooler = PerceiverPooler(d_in=512, d_model=D_MODEL, k=K, num_layers=2, num_heads=4).to(device)
sg_enc = sota_mlp(2, 512, 512, 2).to(device)
q_head = sota_mlp(1024, 512, D, 1).to(device)
k_head = sota_mlp(D_MODEL, D, D, 1).to(device)
mods = [enc, pooler, sg_enc, q_head, k_head]
params = [p for m in mods for p in m.parameters()]
print(f"params {sum(p.numel() for p in params)/1e6:.2f}M | B={B} K={K} d_model={D_MODEL}", flush=True)
opt = torch.optim.Adam(params, lr=1e-3)
TEMP = 0.1
eye = torch.eye(B, dtype=bool, device=device)

def run_step(rng):
    traj, mask, s, g, dist = make_batch(rng)
    Bc, Tc, _ = traj.shape
    feats = enc(traj.reshape(Bc * Tc, 2)).reshape(Bc, Tc, 512)
    et = pooler(feats, key_padding_mask=mask)
    k = F.normalize(k_head(et.mean(1)), dim=1)
    q = F.normalize(q_head(torch.cat([sg_enc(s), sg_enc(g)], 1)), dim=1)
    logits = (q @ k.t()) / TEMP
    labels = torch.arange(Bc, device=device)
    loss = 0.5 * (F.cross_entropy(logits, labels) + F.cross_entropy(logits.t(), labels))
    acc = (logits.argmax(1) == labels).float().mean().item()
    off = (k @ k.t())[~eye].mean().item()
    return loss, acc, off
...
print(f"\nFINAL (fresh): match-acc {np.mean(accs):.3f} (chance {1/B:.3f})  key-offdiag-sim {np.mean(offs):+.3f}", flush=True)
print("PASS if match-acc >> chance AND key-offdiag low (e_target encodes distinct state trajectories, no collapse).", flush=True)
