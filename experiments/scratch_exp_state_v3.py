...
print(f"params {sum(p.numel() for p in params)/1e6:.2f}M | B={B} K={K} d_model={D_MODEL} (NO pool; slot-wise cosine /2)", flush=True)
...
def run_step(rng):
    ...
    An = F.normalize(q, dim=-1); Bn = F.normalize(et, dim=-1)
    sim = torch.einsum("ikd,jkd->ij", An, Bn) / K  # slot-wise cosine, meaned over K slots
    logits = sim / TEMP
    labels = torch.arange(Bc, device=device)
    loss = 0.5 * (F.cross_entropy(logits, labels) + F.cross_entropy(logits.t(), labels))
    acc = (logits.argmax(1) == labels).float().mean().item()
    off = (torch.einsum("ikd,jkd->ij", Bn, Bn) / K)[~eye].mean().item()
    return loss, acc, off
